...
def loadWAV(filename, max_frames, evalmode=True, num_eval=10):
    # print(f"Trying to load: {filename}")  # Debugging line

    if not os.path.exists(filename):
        raise FileNotFoundError(f"File not found: {filename}")  # Error if file doesn't exist
    ...
    audio = audio.squeeze(0).numpy()  # Convert from tensor to numpy array
...
class AugmentWAV(object):
    ...
    def reverberate(self, audio):
        ...
        rir = rir.squeeze(0).numpy()  # Convert to numpy
...
class train_dataset_loader(Dataset):
    def __init__(self, train_list, augment, musan_path, rir_path, max_frames, train_path, **kwargs):
        ...
        for lidx, line in enumerate(lines):
            ...
            filename = os.path.join(train_path, data[1].replace("VoxCeleb/vox1_dev_wav/", ""));
            # filename = os.path.join("/content/data/VoxCeleb/vox1_dev_wav", data[1])
